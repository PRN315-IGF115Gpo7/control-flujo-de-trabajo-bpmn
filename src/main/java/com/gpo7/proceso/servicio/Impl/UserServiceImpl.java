...
public class UserServiceImpl implements UserDetailsService{
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		com.gpo7.proceso.entity.Usuario user = userJpaRepository.findByUsername(username);
		List<GrantedAuthority> authorities = buildAuthorities(user.getRoles());
		userAttemps(user);
		return buildUser(user, authorities);
	}
	
	private User buildUser(com.gpo7.proceso.entity.Usuario user, List<GrantedAuthority> authorities) {
